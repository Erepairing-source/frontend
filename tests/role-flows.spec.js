...
const creds = {
  customer: process.env.PW_USE_HIERARCHY_CREDS
    ? { email: process.env.PW_CUSTOMER_EMAIL || 'customer1@hierarchy.example.com', password: process.env.PW_CUSTOMER_PASSWORD || 'HierarchyTest1!' }
    : { email: 'anand12@anand.com', password: 'qqqqqq' },
  support_engineer: process.env.PW_USE_HIERARCHY_CREDS
    ? { email: process.env.PW_ENGINEER_EMAIL || 'engineer_1@hierarchy.example.com', password: process.env.PW_ENGINEER_PASSWORD || 'HierarchyTest1!' }
    : { email: 'anand10@anand.com', password: 'qqqqqq' },
  city_admin: process.env.PW_USE_HIERARCHY_CREDS
    ? { email: process.env.PW_CITY_ADMIN_EMAIL || 'cityadmin_1@hierarchy.example.com', password: process.env.PW_CITY_ADMIN_PASSWORD || 'HierarchyTest1!' }
    : { email: 'anand14@anand.com', password: 'qqqqqq' },
  state_admin: process.env.PW_USE_HIERARCHY_CREDS
    ? { email: process.env.PW_STATE_ADMIN_EMAIL || 'stateadmin_ka@hierarchy.example.com', password: process.env.PW_STATE_ADMIN_PASSWORD || 'HierarchyTest1!' }
    : { email: 'anand13@anand.com', password: 'qqqqqq' },
  country_admin: process.env.PW_USE_HIERARCHY_CREDS
    ? { email: process.env.PW_COUNTRY_ADMIN_EMAIL || 'countryadmin@hierarchy.example.com', password: process.env.PW_COUNTRY_ADMIN_PASSWORD || 'HierarchyTest1!' }
    : { email: 'anand11@anand.com', password: 'qqqqqq' },
  organization_admin: process.env.PW_USE_HIERARCHY_CREDS
    ? { email: process.env.PW_ORG_ADMIN_EMAIL || 'orgadmin@hierarchy.example.com', password: process.env.PW_ORG_ADMIN_PASSWORD || 'HierarchyTest1!' }
    : { email: 'anand9@anand.com', password: 'qqqqqq' },
...
async function loginAndExpect(page, email, password, pathPart, headingTextOrRegex) {
  await page.goto('/login');
  await page.getByPlaceholder('admin@erepairing.com').fill(email);
  await page.getByPlaceholder('Enter your password').fill(password);
  await page.getByRole('button', { name: 'Sign In' }).click();
  await page.waitForURL(`**${pathPart}`, { timeout: 25000 });
  await expect(page.getByRole('heading', { name: headingTextOrRegex })).toBeVisible();
}
...
test('role dashboards load', async ({ page }) => {
  ...
  await loginAndExpect(page, creds.country_admin.email, creds.country_admin.password, '/country-admin/dashboard', /Country Admin Dashboard|National Dashboard/i);
  await loginAndExpect(page, creds.organization_admin.email, creds.organization_admin.password, '/organization-admin/dashboard', /Organization Admin Dashboard|Organization/i);
...
test('city admin can view feedback tab', async ({ page }) => {
  await page.goto('/login');
  await page.getByPlaceholder('admin@erepairing.com').fill(creds.city_admin.email);
  await page.getByPlaceholder('Enter your password').fill(creds.city_admin.password);
  await page.getByRole('button', { name: 'Sign In' }).click();
  await page.waitForURL('**/city-admin/dashboard');
  await page.waitForTimeout(3000);
  const feedbackTab = page.locator('button[role="tab"]:has-text("Feedback"), [role="tab"]:has-text("Feedback")').first();
  if (await feedbackTab.isVisible({ timeout: 5000 })) {
    await feedbackTab.click();
    await page.waitForTimeout(2000);
    await expect(page.locator('body')).toContainText(/Feedback|Follow-up/i);
  } else {
    // Tab might not be visible, verify dashboard loaded
    await expect(page.getByRole('heading', { name: 'City Admin Dashboard' })).toBeVisible();
  }
...
test('state admin can view cities', async ({ page }) => {
  ...
  await page.waitForTimeout(2000);
  await expect(page.locator('body')).toContainText(/City|Bengaluru|Mumbai|Chennai|Karnataka|Maharashtra|Tamil Nadu/i);
});
...
test('country admin can view states', async ({ page }) => {
  ...
  await page.waitForTimeout(2000);
  await expect(page.locator('body')).toContainText(/State|Karnataka|Maharashtra|Tamil Nadu|Rajasthan/i);
});
