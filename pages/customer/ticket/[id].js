...
export default function CustomerTicketDetail() {
  ...
  const fetchTracking = async () => {
    const token = localStorage.getItem('token')
    try {
      const response = await fetch(`${getApiBase()}/tickets/${id}/tracking`, {
        headers: { Authorization: `Bearer ${token}` }
      })
      const data = await response.json()
      if (!response.ok) {
  ...
  const fetchEstimate = async () => {
    const token = localStorage.getItem('token')
    try {
      const response = await fetch(`${getApiBase()}/tickets/${id}/estimate`, {
        headers: { Authorization: `Bearer ${token}` }
      })
      const data = await response.json()
      if (response.ok) {
  ...
  const fetchAiSummary = async () => {
    const token = localStorage.getItem('token')
    try {
      const response = await fetch((getApiBase()) + '/ai/tickets/summary', {
        method: 'POST',
  ...
  const loadTicket = async () => {
    ...
    try {
      const response = await fetch(`${getApiBase()}/tickets/${id}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      })
      if (response.ok) {
        const data = await response.json()
  ...
  const handleReschedule = async () => {
    ...
    const token = localStorage.getItem('token')
    try {
      const response = await fetch(`${getApiBase()}/tickets/${id}/reschedule`, {
        method: 'POST',
        headers: {
  ...
  const handleSubmitFeedback = async () => {
    const token = localStorage.getItem('token')
    try {
      const response = await fetch(`${getApiBase()}/tickets/${id}/feedback`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            {(ticket.status === 'assigned' || ticket.status === 'in_progress') && (
              <div className="rounded-lg border border-amber-200 bg-amber-50 p-4">
                ...
                <p className="text-sm text-amber-800">
                  If your engineer asked for an OTP, we sent a 6-digit code to your email. Check your inbox (and spam) and share that code with the engineer when they ask. You can also see OTP updates in your{' '}
                  <button
                    type="button"
                    onClick={() => router.push('/customer/dashboard?tab=notifications')}
        ...
        <Card className="mb-6">
          ...
          <CardContent>
            <ComingSoon variant="card" message="AI Ticket Summary – Coming Soon" />
          </CardContent>
        </Card>
        {estimate && (
          <Card className="mb-6">
