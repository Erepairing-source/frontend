...
export default function EngineerDashboard({ user }) {
  ...
  useEffect(() => {
    ...
    fetch((getApiBase()) + '/tickets/?assigned_to_me=true', {
      headers: {
  ...
  const startTicket = async (ticketId) => {
    const token = localStorage.getItem('token')
    try {
      const response = await fetch(`${getApiBase()}/tickets/${ticketId}/start`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`
        }
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        <Button
          ...
          onClick={async () => {
            ...
            try {
              const response = await fetch((getApiBase()) + '/tickets/assigned/calendar', {
                headers: { Authorization: `Bearer ${token}` }
              })
              if (response.ok) {
