...
export default function StateCityDetail() {
  ...
  const loadCityData = async () => {
    ...
    try {
      const citiesRes = await fetch(getApiBase() + '/state-admin/cities', {
      ...
      const engineersRes = await fetch(`${getApiBase()}/state-admin/cities/${cityId}/engineers`, {
        headers: { Authorization: `Bearer ${token}` }
      })
      if (engineersRes.ok) {
      ...
      const allEngineersRes = await fetch(getApiBase() + '/state-admin/engineers/reallocations', {
        headers: { Authorization: `Bearer ${token}` }
      })
      ...
      const inventoryRes = await fetch(`${getApiBase()}/state-admin/cities/${cityId}/inventory`, {
        headers: { Authorization: `Bearer ${token}` }
      })
      if (inventoryRes.ok) {
      ...
      const complaintsRes = await fetch(`${getApiBase()}/state-admin/cities/${cityId}/complaints`, {
        headers: { Authorization: `Bearer ${token}` }
      })
      if (complaintsRes.ok) {
  ...
  const handleBulkReassign = async () => {
    ...
    const token = localStorage.getItem('token')
    try {
      const response = await fetch(getApiBase() + '/state-admin/tickets/bulk-reassign', {
        method: 'POST',
        headers: {
  ...
  const handleFollowUp = async () => {
    ...
    const token = localStorage.getItem('token')
    try {
      const response = await fetch(`${getApiBase()}/state-admin/complaints/${selectedComplaint.id}/follow-up`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card>
            ...
            <CardContent>
              {complaints.length === 0 ? (
                <p className="text-sm text-gray-600">No feedback recorded for this city.</p>
              ) : (
                <div className="space-y-2 text-sm">
              ...
              <CardContent className="space-y-4">
                ...
                <div>
                  ...
                  <Select value={selectedEngineerId} onValueChange={setSelectedEngineerId}>
                    <SelectTrigger>
                      <SelectValue placeholder={engineers.length === 0 ? 'No engineers in this city' : 'Select engineer'} />
                    </SelectTrigger>
                    <SelectContent>
                      {engineers.map(engineer => (
                        <SelectItem key={engineer.id} value={engineer.id.toString()}>
                          {engineer.name} ({engineer.assigned_tickets ?? 0} assigned)
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
              ...
              <CardContent className="space-y-4">
                ...
                <div>
                  ...
                  <Select value={followUpForm.engineer_id || ''} onValueChange={(value) => setFollowUpForm({ ...followUpForm, engineer_id: value })}>
                    ...
                    <SelectContent>
                      {(allEngineers.length ? allEngineers : engineers).map(engineer => (
                        <SelectItem key={engineer.id} value={engineer.id.toString()}>
                          {(engineer.full_name || engineer.name) || ''} {(engineer.city_name || engineer.current_city_name) ? `- ${engineer.city_name || engineer.current_city_name}` : ''}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                ...
                <div className="flex gap-2">
                  <Button
                    onClick={async () => {
                      const token = localStorage.getItem('token')
                      try {
                        const response = await fetch(`${getApiBase()}/state-admin/cities/${city?.id ?? id}/hq`, {
                          method: 'POST',
                          headers: {
                            Authorization: `Bearer ${token}`,
                            'Content-Type': 'application/json'
