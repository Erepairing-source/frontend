...
export default function StatePolicies() {
  ...
  const loadData = async () => {
    ...
    try {
      const policiesRes = await fetch(getApiBase() + '/state-admin/sla-policies', {
  ...
  const handleSave = async () => {
    ...
    try {
      const url = editingPolicy
        ? `${getApiBase()}/state-admin/sla-policies/${editingPolicy.id}`
        : getApiBase() + '/state-admin/sla-policies'
  ...
  const handleDelete = async (policyId) => {
    ...
    try {
      const response = await fetch(`${getApiBase()}/state-admin/sla-policies/${policyId}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` }
      })
      if (response.ok) {
