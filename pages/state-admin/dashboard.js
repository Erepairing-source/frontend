...
export default function StateAdminDashboard() {
  ...
  // When Reallocate Engineer modal opens, refresh engineers list so dropdown is populated
  useEffect(() => {
    if (!showEngineerReallocation) return
    const token = localStorage.getItem('token')
    if (!token) return
    fetch(getApiBase() + '/state-admin/engineers/reallocations', { headers: { 'Authorization': `Bearer ${token}` } })
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      const statsResponse = await fetch(`${getApiBase()}/state-admin/dashboard?time_range=${timeRange}`, {
      ...
      const citiesResponse = await fetch(getApiBase() + '/state-admin/cities', {
        headers: { 'Authorization': `Bearer ${token}` }
      })
      ...
      const engineersRes = await fetch(getApiBase() + '/state-admin/engineers/reallocations', {
        headers: { 'Authorization': `Bearer ${token}` }
      })
      if (engineersRes.ok) {
      ...
      const inventoryRes = await fetch(getApiBase() + '/state-admin/inventory/parts', {
        headers: { 'Authorization': `Bearer ${token}` }
      })
      if (inventoryRes.ok) {
  ...
  const loadSlaRisk = async () => {
    ...
    try {
      const response = await fetch(`${getApiBase()}/state-admin/sla-risk?${params.toString()}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      })
      if (response.ok) {
  ...
  const loadComplianceAlerts = async () => {
    ...
    try {
      const response = await fetch(getApiBase() + '/state-admin/compliance-alerts', {
        headers: { 'Authorization': `Bearer ${token}` }
      })
      if (response.ok) {
  ...
  const loadDemandForecast = async () => {
    ...
    if (!token) return
    try {
      const response = await fetch(getApiBase() + '/state-admin/demand-forecast', {
        headers: { 'Authorization': `Bearer ${token}` }
      })
      if (response.ok) {
  ...
  const loadTrainingGaps = async () => {
    ...
    if (!token) return
    try {
      const response = await fetch(getApiBase() + '/state-admin/training-gaps', {
        headers: { 'Authorization': `Bearer ${token}` }
      })
      if (response.ok) {
  ...
  const runPolicyImpact = async () => {
    ...
    if (!token) return
    try {
      const response = await fetch(getApiBase() + '/state-admin/policy-impact', {
        method: 'POST',
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <DashboardHeader
          ...
          subtitle="Multi-city health monitoring and resource management"
        >
          <Link href="/state-admin/tickets" className="text-sm font-semibold text-indigo-700 hover:text-indigo-900 hover:underline">
              ...
              <CardContent className="space-y-4">
                ...
                <div>
                  ...
                  <Select value={transferForm.to_city_id} onValueChange={(val) => setTransferForm({...transferForm, to_city_id: val})}>
                    ...
                    <SelectContent>
                      {cities.filter(c => c.id != null && String(c.id) !== transferForm.from_city_id).map(city => (
                ...
                <div>
                  ...
                  <Select value={transferForm.part_id} onValueChange={(val) => setTransferForm({...transferForm, part_id: val})}>
                    ...
                    <SelectContent>
                      {inventoryParts.filter(p => p.city_id != null && String(p.city_id) === transferForm.from_city_id).map(part => (
                        <SelectItem key={part.part_id ?? part.sku} value={part.part_id != null ? String(part.part_id) : ''}>
                ...
                <div className="flex gap-2">
                  <Button
                    onClick={async () => {
                      const token = localStorage.getItem('token')
                      try {
                        const response = await fetch(getApiBase() + '/state-admin/inventory/transfer', {
                          method: 'POST',
                          headers: {
              ...
              <CardContent className="space-y-4">
                <div>
                  ...
                  <Select value={reallocationForm.engineer_id} onValueChange={(val) => setReallocationForm({...reallocationForm, engineer_id: val})}>
                    <SelectTrigger>
                      <SelectValue placeholder={engineers.length === 0 ? 'No engineers in your state' : 'Select engineer'} />
                    </SelectTrigger>
                    <SelectContent>
                      {engineers.map(engineer => (
                        <SelectItem key={engineer.id} value={engineer.id != null ? String(engineer.id) : ''}>
                          {(engineer.full_name || engineer.name) || 'Engineer'} – {(engineer.city_name || engineer.current_city_name) || 'No city'}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                  {engineers.length === 0 && (
                    <p className="text-sm text-amber-600 mt-1">Add support engineers in your state (same organization) to reallocate them.</p>
                  )}
                </div>
                <div>
                  <Label>To City</Label>
                ...
                <div className="flex gap-2">
                  <Button
                    onClick={async () => {
                      const token = localStorage.getItem('token')
                      try {
                        const response = await fetch(getApiBase() + '/state-admin/engineers/reallocate', {
                          method: 'POST',
                          headers: {
                            'Authorization': `Bearer ${token}`,
                            'Content-Type': 'application/json'
