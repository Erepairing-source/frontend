...
export default function CountryAdminTickets() {
  ...
  useEffect(() => {
    ...
    if (!token) return
    fetch(getApiBase() + '/country-admin/states', {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (selectedState !== 'all') ticketParams.append('state_id', selectedState)
      const escParams = new URLSearchParams()
      if (selectedState !== 'all') escParams.append('state_id', selectedState)

      const ticketUrl = `${getApiBase()}/tickets/?${ticketParams.toString()}`
      const escUrl =
        escParams.toString().length > 0
          ? `${getApiBase()}/country-admin/escalations?${escParams.toString()}`
          : `${getApiBase()}/country-admin/escalations`
  ...
  const statesWithId = states.filter((s) => s.id != null)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-6">
          <Button variant="ghost" size="sm" className="mb-2 -ml-2" onClick={() => router.push('/country-admin/dashboard')}>
            <ArrowLeft className="w-4 h-4 mr-1" />
            Dashboard
          </Button>
          <h1 className="text-3xl font-bold text-gray-900">Country tickets & escalations</h1>
          <p className="text-gray-600 mt-1">
            Scoped to your country (and partner orgs when configured). Filter by state to align with the national dashboard.
            Tickets from support agents use customer geography and appear under the same rules.
          </p>
        </div>
        ...
        <Card className="p-4 mb-6">
          <div className="flex flex-wrap items-center gap-3">
            ...
            <Select value={selectedState} onValueChange={setSelectedState}>
              ...
              <SelectContent>
                <SelectItem value="all">All states in country</SelectItem>
                {statesWithId.map((s) => (
