...
const COLORS = ['#0ea5e9', '#7c3aed', '#f59e0b', '#22c55e', '#ef4444']

export default function CountryAdminAnalyticsPage() {
  ...
  const load = async () => {
    ...
    const res = await fetch(`${getApiBase()}/country-admin/analytics?time_range=${timeRange}`, {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-sky-50/20 to-violet-50/20">
      <div className="max-w-7xl mx-auto px-4 py-8 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <Button variant="ghost" onClick={() => router.push('/country-admin/dashboard')} className="-ml-2 mb-2">
              <ArrowLeft size={16} className="mr-2" /> Back
            </Button>
            <h1 className="text-3xl font-bold text-slate-900 flex items-center gap-2"><BarChart3 className="text-sky-600" /> Country Analytics</h1>
          </div>
