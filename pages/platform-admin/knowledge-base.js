...
export default function PlatformKnowledgeBase() {
  ...
  const loadEntries = async () => {
    ...
    try {
      const response = await fetch((getApiBase()) + '/ai/knowledge-base', {
  ...
  const saveEntry = async () => {
    ...
    try {
      const response = await fetch((getApiBase()) + '/ai/knowledge-base/upsert', {
        method: 'POST',
  ...
  const deleteEntry = async (id) => {
    ...
    try {
      const response = await fetch(`${getApiBase()}/ai/knowledge-base/${id}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` }
      })
      if (response.ok) {
