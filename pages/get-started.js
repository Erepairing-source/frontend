    ...
    fetch(getApiBase() + '/platform-admin/plans/public')
      ...
      .then(data => {
        if (Array.isArray(data)) {
          setPlans(data.filter(p => p.is_active && p.is_visible))
        }
      })
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (existingToken && existingRole === 'vendor') {
          ...
          if (window.location.pathname.includes('/get-started') || window.location.pathname.includes('/signup')) {
          ...
          alert(
            data.message ||
              'Check your email for your verification code and link to set your password.'
          )
          router.push('/login')
        } else {
